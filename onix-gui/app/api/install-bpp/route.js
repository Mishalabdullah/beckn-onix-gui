...
export async function GET(req, res) {
  try {
    ...
    // }
    let updateBppConfigCommand = `bash /tmp/beckn-onix/install/scripts/update_bpp_config.sh  ${registryUrl} ${bppSubscriberId}  ${bppSubscriberUrl} ${webhookUrl}`;
    ...
    const result3 = await executeCommand(
      ' docker-compose -f /tmp/beckn-onix/install/docker-compose-v2.yml up -d  "bpp-client"'
    );
    console.log("Result 3:", result3);

    const result4 = await executeCommand(
      ' docker-compose -f /tmp/beckn-onix/install/docker-compose-v2.yml up -d $1 "bpp-network"'
    );
